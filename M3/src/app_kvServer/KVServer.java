...
public class KVServer extends Thread implements IKVServer {
    ...
    public void putKV(String key, String value) throws Exception{
		...
		// Only send replicated PUT or DELETE command to successors if data is in range
		// This means the curent server is the coordinator
		// Or else the server is the successor to receive PUT or DELETE command.
		if (metadata.inRange(key, serverName)) {
			putOrDeleteReplicatedData(key, value);
			logger.debug("send replication to successors, if possible.");
		}
	}
  ...
  public ServerStatus getStatus() {
	...
	public boolean putOrDeleteReplicatedData(String key, String value) {
		...
		for (int i = 0; i < targetedServersNames.size(); i++) {
			...
			String address = ip_port_array[0];
			int port = Integer.parseInt(ip_port_array[1]);
			KVMessage message = new KVMessageImple(key, value, KVMessage.StatusType.PUT, true);
			// Response Object placeholder.

			KVMessageImple received_message = new KVMessageImple(key, null, KVMessage.StatusType.PUT, true);
			try 
			{
				// Establish connection to targeted server
				connectToTargetedServer(address, port);
			...
			received_message.getStatus().equals(KVMessage.StatusType.DELETE_ERROR)) {
				...
				logger.error("Received unknown status regarding replicated PUT / DELETE: " + received_message.getStatus());
				return false;
			}			
		}
		return true;
