...
public class ClientConnection implements Runnable {
	...
	public TextMessage receiveMessage() throws IOException {
		...
		logger.debug(kvServer.serverName + ": RECEIVE \t<" + clientSocket.getInetAddress().getHostAddress() + ":" + clientSocket.getPort()
				+ ">: '" + msg.getMsg().trim() + "'");
		return msg;
	}
			...
			switch (obj_mess.getStatus()) {
				...
				case GET: {
					...
					try {
						...
						if (!kvServer.getMetadata().inRange(obj_mess.getKey(), kvServer.serverName)
								&& !kvServer.getMetadata().inReplicaRange(obj_mess.getKey(), kvServer.serverName)) {
							sendMetadataToClient();
				...
				case PUT: {
					...
					try {
						...
						} else if (!kvServer.getMetadata().inRange(obj_mess.getKey(), kvServer.serverName)) {
							System.out.println(
						...
						if (obj_mess.getValue() == null) {
							...
							logger.error("Failed DELETE: key = " + obj_mess.getKey() + ", value = " + obj_mess.getValue(),
									e);
						} else {
							...
							response.setValue("Failed UPDATE/INSERT: key = " + obj_mess.getKey() + ", value = "
									+ obj_mess.getValue());
							logger.error(
									"Failed UPDATE/INSERT: key = " + obj_mess.getKey() + ", value = " + obj_mess.getValue(),
									e);
						}
					}
					break;
