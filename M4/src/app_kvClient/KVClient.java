...
import client.ConnectionMonitor2;
...
public class KVClient implements IKVClient, ClientSocketListener {
	...
	private KVSubscription sub_client = null;
	private String clientName;

    private String serverAddress;
	private int serverPort;
	private ConnectionMonitor monitor;
	private ConnectionMonitor2 monitor2;
    ...
    public void run() throws Exception {
		while(!stop) {
			...
			try {
				...
				logger.debug("Current connection lost, try to connect to another server");
				client.reconnect();
			}
		}
    }
    ...
    private void handleCommand(String cmdLine) throws Exception {
		...
		if(tokens[0].equals("quit")) {	
			...
			if (clientName == null){
				printError("Please set a client name first");
				return;
			}
			...
			if (client!= null && client.isRunning()){
				if (checkCommandLength(tokens.length, 2, 2)) {
					logger.info("Subscribe " + tokens[1]);
			...
			if (client!= null && client.isRunning()){
				if (checkCommandLength(tokens.length, 2, 2)) {
					logger.info("Unsubscribe " + tokens[1]);
					client.unsubscribe(tokens[1]);
				}
			} else {
				printError("No valid connection with server. Please connect to a server!");
			}
			...
			if (checkCommandLength(tokens.length, 2, 2)) {
					logger.info("set name: " + tokens[1]);
					clientName = tokens[1];
					PROMPT = clientName + "> ";
			}

	    } else if (tokens[0].equals("didSub")){
			if (checkCommandLength(tokens.length, 2, 2)) {
				logger.info("checking subscription: " + tokens[1]);
				client.checkSubscription(tokens[1]);
			}
		} else if (tokens[0].equals("subtable")){
			if (checkCommandLength(tokens.length, 2, 2)) {
				logger.info("Fetching SubconnectionTable");
				client.getSubconnectionTable(tokens[1]);
			}
    ...
    public void newConnection(String hostname, int port) throws Exception{
        client = new KVStore(hostname, port, clientName);
		client.addListener(this);
		client.connect();
		sub_client = new KVSubscription(hostname, port, clientName);
		sub_client.addListener(this);
		sub_client.connect();
		monitor = new ConnectionMonitor(client, sub_client, clientName);
		monitor.start();
		monitor2 = new ConnectionMonitor2(client, sub_client, clientName);
		monitor2.start();
	}
    ...
    public static void main(String[] args) {
    	try {
			new LogSetup("logs/KVClient.log", Level.DEBUG);
