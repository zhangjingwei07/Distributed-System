...
public class KVStorage {
    ...
    private final String SEND_SUFFIX = "_send";
    private final String REMAIN_SUFFIX = "_remain";
    private String FILE_DIRECTORY = null;
    private String getStorageFileName = null;

    public KVStorage(int port){
        logger = Logger.getLogger(port + "_storage");
        // Get current directroy of runnin jar file
        try{
            FILE_DIRECTORY = new File(KVStorage.class.getProtectionDomain().getCodeSource().getLocation().toURI()).getParent();
        } catch (Exception e){
            logger.error("error in getting jar file path, "+ e);
        }
        cache = null;
        setStorageFileName(Integer.toString(port) + DATA_INFIX);
    }
    ...
    public synchronized int put(String key, String value){
        ...
        try { 
            ...
            FileReader reader = new FileReader(getStorageFileWritingPath(getStorageFileName()));
        ...
        try (FileWriter file = new FileWriter(getStorageFileWritingPath(getStorageFileName()))) {
    ...
    public synchronized String get(String key){
        ...
        JSONParser jsonParser = new JSONParser();
        try (FileReader reader = new FileReader(getStorageFileWritingPath(getStorageFileName()))) {
            //Read json file
            value_pairs = (JSONObject)jsonParser.parse(reader);
    ...
    public synchronized void clearStorage(){
        ...
        try{
            new FileWriter(getStorageFileWritingPath(getStorageFileName())).close();
        }
        catch( IOException e) {
            e.printStackTrace();
        }
    ...
    public String getStorageFileWritingPath(String getStorageFileName) {
        return FILE_DIRECTORY + "/data/" + getStorageFileName + ".json";
    }

    public String getStorageFileNameWithSuffix(String suffix) {
        return this.getStorageFileName + suffix;
    }
    ...
    public void setStorageFileName(String getStorageFileName) {
        this.getStorageFileName = getStorageFileName;
    }
    ...
    public synchronized String prevMoveData(String leftBound, String rightBound) {
        ...
        if (remain_value_pairs.isEmpty()) {
            ...
            try (FileWriter remainStorageFileWriter = new FileWriter(getStorageFileWritingPath(getStorageFileNameWithSuffix(REMAIN_SUFFIX)))) {
    ...
    public synchronized void postMoveData() {
        File oriStorageFile = new File(getStorageFileWritingPath(getStorageFileName()));
        ...
        File remainStorageFile = new File(getStorageFileWritingPath(getStorageFileNameWithSuffix(REMAIN_SUFFIX)));
        if (!remainStorageFile.exists()) {
            logger.debug("The remain storage file does not exist, meaning no remain data.");
            return;
        }

        if (remainStorageFile.renameTo(new File(getStorageFileWritingPath(getStorageFileName())))) {
